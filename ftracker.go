...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case "Плавание":
		...
		speed := swimmingMeanSpeed(lengthPool, countPool, duration) * kmhInMsec
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeed := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	meanSpeed := swimmingMeanSpeed(lengthPool, countPool, duration) * kmhInMsec
